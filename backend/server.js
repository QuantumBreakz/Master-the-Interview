...
app.post('/api/interview/code-result', async (req, res) => {
    try {
        ...
        // BUT FIRST: If we have an AI Analysis service running, let's check the code!
        let aiAnalysisResult = null;
        try {
            // Only analyze if code length is sufficient to be meaningful
            if (code && code.length > 50) {
                ...
                if (analysisResponse.ok) {
                    aiAnalysisResult = await analysisResponse.json();
                    console.log('🤖 AI Analysis Result:', aiAnalysisResult);
                }
            }
        } catch (err) {
            console.warn('⚠️ AI Analysis service unavailable or error:', err.message);
        }

        // Add analysis insight to the conversation for the AI Interviewer
        if (aiAnalysisResult) {
            const isAi = aiAnalysisResult.is_ai;
            const confidence = (aiAnalysisResult.confidence * 100).toFixed(1);
            const characteristics = isAi
                ? aiAnalysisResult.analysis.ai_characteristics.slice(0, 3).join(', ')
                : aiAnalysisResult.analysis.human_characteristics.slice(0, 3).join(', ');

            const analysisMessage = `
[SYSTEM NOTICE: Automated Code Analysis]
The code submitted by the candidate was analyzed.
Prediction: ${isAi ? 'AI-GENERATED' : 'HUMAN-WRITTEN'} (${confidence}% confidence).
Key characteristics observed: ${characteristics || 'None specific'}.

INSTRUCTION: Use this information to tailor your response.
- If it looks AI-generated and they didn't explain it well, ask deeper questions about WHY they wrote it that way to verify understanding.
- If it looks human-written but messy, ask about cleanup/refactoring.
- Do NOT explicitly accuse them of cheating unless you are asking a clarification question like "Can you explain how this specific part works?"
`;
            conversation.push({ role: 'system', content: analysisMessage.trim() });
            ...
            try {
                ...
                console.warn('Failed to save AI analysis to metadata:', e);
            }
        }
        ...
        res.json({ success: true, aiResponse, aiAnalysis: aiAnalysisResult });
    } catch (error) {
