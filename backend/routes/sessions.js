...
// Get full session results (for Results Page)
router.get('/:sessionId/results', async (req, res) => {
  try {
    const { sessionId } = req.params;
    // In a real app, you would verify the user is the candidate or recruiter
    // For now, we will allow it if the session is completed or expired
    ...
    if (session.sessionStatus !== 'completed' && session.sessionStatus !== 'expired') {
      // Optional: restrict access to ongoing sessions
      // return res.status(403).json({ success: false, error: 'Session is not yet complete' });
    }
    ...
    res.json({
      ...
      data: {
        ...
        candidateName: session.candidateDetails.candidateName,
        role: session.candidateDetails.role,
        date: session.sessionConfig.scheduledStartTime,
        duration: session.accessControl.totalTimeSpent || 0,
        questionsAsked: session.interviewData?.metadata?.questionsAsked || 0,
        transcript: session.interviewData?.conversationHistory || [],
        feedback: session.interviewData?.results?.resultSummary || "Pending AI Analysis...",
        scores: {
          technical: 85, // Mock score for now if not in DB
          communication: 90,
          problemSolving: 88
        }
      }
    ...
    res.status(500).json({
      success: false,
      error: 'Failed to retrieve session results'
    });
  }
});
